...
def create_demographic_comparison_chart(
    data, x_col, y_col, comparison_col, title, colors
):
    """
...
def show(data, filters, colors, fuente_poblacion="DANE"):
    ...
    filtered_data["vacunacion"] = normalize_demographic_data(
    ...
    edad_stats = calculate_demographic_stats(
        filtered_data["vacunacion"], "Grupo_Edad", "Edad"
    )
    genero_stats = calculate_demographic_stats(
        filtered_data["vacunacion"], "Sexo", "Género"
    )
    etnia_stats = calculate_demographic_stats(
        filtered_data["vacunacion"], "GrupoEtnico", "Etnia"
    )
                ...
                try:
                    edad_data["Grupo_Edad"] = pd.Categorical(
                        ...
                        categories=[
                            ...
                            if g in edad_data["Grupo_Edad"].values
                        ]
                        + [
                            g
                            for g in edad_data["Grupo_Edad"].values
                            if g not in orden_grupos
                        ],
                        ordered=True,
                    )
                ...
                if "Sexo" in filtered_data["vacunacion"].columns:
                    ...
                    for grupo in edad_counts.index:
                        if grupo != "Sin dato":
                            ...
                            masculino = len(
                                grupo_data[grupo_data["Sexo"] == "Masculino"]
                            )
                            femenino = len(grupo_data[grupo_data["Sexo"] == "Femenino"])
                            no_binario = len(
                                grupo_data[grupo_data["Sexo"] == "No Binario"]
                            )
    ...
    with col1:
        ...
        if genero_stats and genero_stats["data"] is not None:
            try:
                ...
                masculino_pct = genero_stats["data"][
                    genero_stats["data"]["Sexo"] == "Masculino"
                ]["Porcentaje"].sum()
                femenino_pct = genero_stats["data"][
                    genero_stats["data"]["Sexo"] == "Femenino"
                ]["Porcentaje"].sum()
                no_binario_pct = genero_stats["data"][
                    genero_stats["data"]["Sexo"] == "No Binario"
                ]["Porcentaje"].sum()
                sin_dato_pct = genero_stats["data"][
                    genero_stats["data"]["Sexo"] == "Sin dato"
                ]["Porcentaje"].sum()
    ...
    with col2:
        ...
        if (
            ...
            and "Grupo_Edad" in filtered_data["vacunacion"].columns
        ):
            try:
    ...
    with col2:
        ...
        try:
            ...
            if "Desplazado" in filtered_data["vacunacion"].columns:
                desplazado_counts = filtered_data["vacunacion"][
                    "Desplazado"
                ].value_counts()
                ...
                vulnerabilidad_data.extend(
                    [
                        ...
                        {
                            "Categoria": "Desplazado - Sin dato",
                            "Cantidad": desplazados_sin_dato,
                        },
                    ]
                )
            ...
            if "Discapacitado" in filtered_data["vacunacion"].columns:
                discapacidad_counts = filtered_data["vacunacion"][
                    "Discapacitado"
                ].value_counts()
                ...
                vulnerabilidad_data.extend(
                    [
                        ...
                        {
                            "Categoria": "Discapacidad - Sin dato",
                            "Cantidad": discapacidad_sin_dato,
                        },
                    ]
                )
            ...
            if vulnerabilidad_data:
                ...
                pct_desplazados = (
                    (desplazados_si / total_registros * 100)
                    if "desplazados_si" in locals()
                    else 0
                )
                pct_discapacidad = (
                    (discapacidad_si / total_registros * 100)
                    if "discapacidad_si" in locals()
                    else 0
                )
    ...
    analisis_seleccionado = st.selectbox(
        "Seleccione el tipo de análisis cruzado:", analisis_opciones
    )
    ...
    try:
        if analisis_seleccionado == "Género vs Grupo Étnico":
            if (
                "Sexo" in filtered_data["vacunacion"].columns
                and "GrupoEtnico" in filtered_data["vacunacion"].columns
            ):
            ...
            if (
                "Grupo_Edad" in filtered_data["vacunacion"].columns
                and "Desplazado" in filtered_data["vacunacion"].columns
            ):
        ...
        try:
            if (
                ...
                try:
                    ...
                    grupos_ordenados = [g for g in orden_grupos if g in grupos_edad]
                    grupos_otros = sorted(
                        [g for g in grupos_edad if g not in orden_grupos]
                    )
                    grupos_edad_final = grupos_ordenados + grupos_otros
                    existing_groups = [
                        g for g in orden_grupos if g in tabla_cruzada.index
                    ]
                    if existing_groups:
                        tabla_cruzada = tabla_cruzada.reindex(
                            existing_groups + ["Total"]
                        )
                except:
                    pass
                ...
                total_sin_total = tabla_cruzada.drop("Total", axis=0).drop(
                    "Total", axis=1
                )
                ...
                generos_con_datos = [
                    col for col in total_sin_total.columns if col != "Sin dato"
                ]
                if len(generos_con_datos) >= 2:
                    masculino_total = total_sin_total.get(
                        "Masculino", pd.Series()
                    ).sum()
                    ...
                    if masculino_total > femenino_total:
                        diferencia = (
                            (masculino_total - femenino_total) / femenino_total * 100
                        )
                        insights.append(
                            f"👨 **Predominio masculino** del {diferencia:.1f}% en la vacunación"
                        )
                    elif femenino_total > masculino_total:
                        diferencia = (
                            (femenino_total - masculino_total) / masculino_total * 100
                        )
                        insights.append(
                            f"👩 **Predominio femenino** del {diferencia:.1f}% en la vacunación"
                        )
                    else:
                ...
                if len(total_sin_total) > 0:
                    ...
                    pct_mas_activo = (
                        cantidad_mas_activo / total_sin_total.sum().sum() * 100
                    )
                    insights.append(
                        f"🎯 **Grupo más activo:** {grupo_mas_activo} ({pct_mas_activo:.1f}% del total)"
                    )
    ...
    try:
        ...
        total_registros = len(filtered_data["vacunacion"])
        registros_sin_dato_genero = len(
            filtered_data["vacunacion"][
                filtered_data["vacunacion"]["Sexo"] == "Sin dato"
            ]
        )
        registros_sin_dato_edad = len(
            filtered_data["vacunacion"][
        ...
        if pct_sin_genero > 10:
            insights_finales.append(
                f"⚠️ **{pct_sin_genero:.1f}% de registros sin datos de género** - revisar calidad de datos"
            )

        if pct_sin_edad > 10:
            insights_finales.append(
                f"⚠️ **{pct_sin_edad:.1f}% de registros sin datos de edad** - revisar calidad de datos"
            )

        # Insight sobre diversidad
        if etnia_stats and etnia_stats["diversity"] > 1.5:
            insights_finales.append(
                f"🌍 **Alta diversidad étnica** (Shannon: {etnia_stats['diversity']:.2f}) - población heterogénea"
            )
        elif etnia_stats and etnia_stats["diversity"] < 0.5:
            insights_finales.append(
                f"🏛️ **Baja diversidad étnica** (Shannon: {etnia_stats['diversity']:.2f}) - población homogénea"
            )
        ...
        if insights_finales:
            ...
            st.info(
                "📊 Los datos demográficos muestran una distribución balanceada sin patrones de alerta."
            )
