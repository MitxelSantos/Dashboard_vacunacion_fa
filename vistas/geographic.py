...
from src.visualization.charts import create_bar_chart, create_scatter_plot, create_line_chart
...
def show(data, filters, colors, fuente_poblacion="DANE"):
    ...
    with col_left:
        ...
        municipios_sorted = filtered_data["metricas"].sort_values(fuente_poblacion, ascending=False)
        municipios_sorted['pct_poblacion'] = (municipios_sorted[fuente_poblacion] / total_poblacion * 100).cumsum()
        municipios_sorted['pct_vacunados'] = (municipios_sorted["Vacunados"] / total_vacunados * 100).cumsum()
    ...
    filtered_data["metricas"]["Eficiencia"] = (
        filtered_data["metricas"]["Vacunados"] / 
        filtered_data["metricas"][fuente_poblacion] * 1000
    ).round(2)
    ...
    filtered_data["metricas"]["Categoria_Tamaño"] = filtered_data["metricas"][fuente_poblacion].apply(categorize_by_size)

    # Análisis por categoría de tamaño
    eficiencia_por_categoria = filtered_data["metricas"].groupby("Categoria_Tamaño").agg({
    ...
    with col_left:
        ...
        categoria_stats = filtered_data["metricas"].groupby("Categoria_Tamaño")[["Eficiencia", cobertura_col]].mean().reset_index()
        categoria_stats.columns = ["Categoria", "Eficiencia_Promedio", "Cobertura_Promedio"]
    ...
    resumen_estadistico = pd.DataFrame({
        ...
        "N° Municipios": [
            len(filtered_data["metricas"][filtered_data["metricas"]["Categoria_Tamaño"] == "Pequeño"]),
            len(filtered_data["metricas"][filtered_data["metricas"]["Categoria_Tamaño"] == "Mediano"]),
            len(filtered_data["metricas"][filtered_data["metricas"]["Categoria_Tamaño"] == "Grande"]),
            len(filtered_data["metricas"])
        ],
        "Población Promedio": [
            filtered_data["metricas"][filtered_data["metricas"]["Categoria_Tamaño"] == "Pequeño"][fuente_poblacion].mean(),
            filtered_data["metricas"][filtered_data["metricas"]["Categoria_Tamaño"] == "Mediano"][fuente_poblacion].mean(),
            filtered_data["metricas"][filtered_data["metricas"]["Categoria_Tamaño"] == "Grande"][fuente_poblacion].mean(),
            filtered_data["metricas"][fuente_poblacion].mean()
        ],
        "Cobertura Promedio (%)": [
            filtered_data["metricas"][filtered_data["metricas"]["Categoria_Tamaño"] == "Pequeño"][cobertura_col].mean(),
            filtered_data["metricas"][filtered_data["metricas"]["Categoria_Tamaño"] == "Mediano"][cobertura_col].mean(),
            filtered_data["metricas"][filtered_data["metricas"]["Categoria_Tamaño"] == "Grande"][cobertura_col].mean(),
            filtered_data["metricas"][cobertura_col].mean()
        ],
        "Eficiencia Promedio": [
            filtered_data["metricas"][filtered_data["metricas"]["Categoria_Tamaño"] == "Pequeño"]["Eficiencia"].mean(),
            filtered_data["metricas"][filtered_data["metricas"]["Categoria_Tamaño"] == "Mediano"]["Eficiencia"].mean(),
            filtered_data["metricas"][filtered_data["metricas"]["Categoria_Tamaño"] == "Grande"]["Eficiencia"].mean(),
            filtered_data["metricas"]["Eficiencia"].mean()
        ]
    })

    # Formatear la tabla
    resumen_estadistico["Población Promedio"] = resumen_estadistico["Población Promedio"].apply(lambda x: f"{x:,.0f}".replace(",", "."))
    resumen_estadistico["Cobertura Promedio (%)"] = resumen_estadistico["Cobertura Promedio (%)"].apply(lambda x: f"{x:.1f}%")
    resumen_estadistico["Eficiencia Promedio"] = resumen_estadistico["Eficiencia Promedio"].apply(lambda x: f"{x:.1f}")
    ...
    tabla_final = filtered_data["metricas"][
        ["DPMP", fuente_poblacion, "Vacunados", cobertura_col, pendientes_col, "Eficiencia", "Categoria_Tamaño"]
    ].copy()
    ...
    cv_cobertura = (cobertura_stats['Desviación Estándar'] / cobertura_stats['Promedio']) * 100
    if cv_cobertura > 30:
        insights.append(f"🔍 **Alta variabilidad** en cobertura entre municipios (CV: {cv_cobertura:.1f}%)")
    else:
        insights.append(f"📈 **Cobertura relativamente homogénea** entre municipios (CV: {cv_cobertura:.1f}%)")
    
    # Insight sobre correlación
    if abs(correlation) > 0.7:
        insights.append(f"📊 **Fuerte correlación** entre población y vacunados (r={correlation:.3f})")
    elif abs(correlation) < 0.3:
        insights.append(f"⚠️ **Débil correlación** entre población y vacunados - revisar estrategias por municipio")
    
    # Insight sobre municipios prioritarios
    if len(municipios_baja) > 0:
        insights.append(f"🎯 **{len(municipios_baja)} municipios requieren atención prioritaria** (cobertura <50%)")
