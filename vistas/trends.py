...
def create_advanced_time_series(
    data, title, colors, show_trend=True, show_anomalies=True
):
    """
            ...
            if len(analysis["picos"]) > 0:
                fig.add_trace(
                    go.Scatter(
                        ...
                        marker=dict(
                            color=colors["success"], size=8, symbol="triangle-up"
                        ),
            ...
            if len(analysis["valles"]) > 0:
                fig.add_trace(
                    go.Scatter(
                        ...
                        marker=dict(
                            color=colors["warning"], size=8, symbol="triangle-down"
                        ),
        ...
        try:
            ...
            if fechas_validas.sum() > 0:
                ...
                process_temporal_analysis(
                    filtered_data, colors, fuente_poblacion, filters
                )
            else:
                st.error(
...
def process_temporal_analysis(filtered_data, colors, fuente_poblacion, filters):
    ...
    with col4:
        ...
        eficiencia = (
            total_vacunados / dias_habiles_aprox if dias_habiles_aprox > 0 else 0
        )

        st.metric(
            "Eficiencia estimada", f"{eficiencia:.1f} vac/día hábil", delta="Lun-Vie"
        )
    ...
    if len(vacunacion_diaria) >= 7:
        ...
        with col_left:
            ...
            if analysis["trend_slope"] > 0:
                ...
                trend_description = (
                    "Sin cambios significativos en el ritmo de vacunación"
                )
    ...
    with col1:
        ...
        filtered_data["vacunacion"]["DiaSemana"] = filtered_data["vacunacion"][
            "FechaVacunacion"
        ].dt.day_name()
        ...
        filtered_data["vacunacion"]["DiaSemana_ES"] = filtered_data["vacunacion"][
            "DiaSemana"
        ].map(dias_mapping)
        ...
        vacunacion_semanal["DiaSemana"] = pd.Categorical(
            vacunacion_semanal["DiaSemana"], categories=orden_dias, ordered=True
        )
        ...
        dia_mas_activo = vacunacion_semanal.loc[
            vacunacion_semanal["Vacunados"].idxmax(), "DiaSemana"
        ]
        dia_menos_activo = vacunacion_semanal.loc[
            vacunacion_semanal["Vacunados"].idxmin(), "DiaSemana"
        ]
    ...
    with col2:
        ...
        if len(vacunacion_mensual) > 1:
            vacunacion_mensual["Crecimiento"] = (
                vacunacion_mensual["Vacunados"].pct_change() * 100
            )
    ...
    analisis_demografico = st.selectbox(
        "Seleccione el análisis demográfico temporal:", analisis_demografico_opciones
    )
        ...
        if analisis_demografico == "Evolución por Género":
            if "Sexo" in filtered_data["vacunacion"].columns:
                ...
                genero_temporal = (
                    filtered_data["vacunacion"]
                    .groupby(
                        [filtered_data["vacunacion"]["FechaVacunacion"].dt.date, "Sexo"]
                    )
                    .size()
                    .reset_index()
                )
            ...
            if "Grupo_Edad" in filtered_data["vacunacion"].columns:
                ...
                edad_temporal_filtrada = edad_temporal[
                    edad_temporal["GrupoEdad"].isin(top_grupos)
                ]
                ...
                fig_edad_temp.update_layout(plot_bgcolor="white", paper_bgcolor="white")
            ...
            if "NombreMunicipioResidencia" in filtered_data["vacunacion"].columns:
                ...
                municipio_temporal["Fecha"] = pd.to_datetime(
                    municipio_temporal["Fecha"]
                )
    ...
    if len(vacunacion_diaria) > 14:  # Necesitamos suficientes datos para proyecciones
        ...
        with col_proj1:
            ...
            if len(ultimos_30_dias) >= 7:
                slope_recent, intercept_recent, r_value_recent, _, _ = stats.linregress(
                    x_recent, y_recent
                )
                ...
                dias_proyeccion = 30
                x_future = np.arange(
                    len(vacunacion_diaria), len(vacunacion_diaria) + dias_proyeccion
                )

                # Usar la tendencia reciente para proyección
                x_for_projection = np.arange(
                    len(ultimos_30_dias), len(ultimos_30_dias) + dias_proyeccion
                )
        ...
        with col_proj2:
            ...
            if total_poblacion > 0:
                ...
                cobertura_actual = (
                    vacunacion_diaria["Vacunados_Acumulados"].iloc[-1]
                    / total_poblacion
                    * 100
                )
                ...
                if estimaciones:
                    ...
                    for est in estimaciones:
                        if est["dias"] > 0:
                            ...
                            info_text += (
                                f"- **{est['meta']}% cobertura:** ¡Ya alcanzada!\n"
                            )

                    st.info(info_text)
                else:
                    st.info(
                        "Las metas de cobertura ya han sido alcanzadas o no se pueden calcular."
                    )

    else:
        st.info(
            "Se necesitan más datos históricos (>14 días) para generar proyecciones confiables."
        )
    ...
    try:
        ...
        if len(vacunacion_semanal) >= 5:  # Al menos datos de 5 días
            ...
            if promedio_laborales > promedio_weekend * 1.5:
                insights_temporales.append(
                    "📈 **Fuerte patrón laboral** - 50% más vacunación en días hábiles"
                )
            elif promedio_weekend > promedio_laborales:
                insights_temporales.append(
                    "🏖️ **Patrón de fin de semana** - mayor actividad los fines de semana"
                )

        # Insight sobre consistencia
        if len(vacunacion_diaria) >= 14:
            cv = (
                vacunacion_diaria["Vacunados"].std()
                / vacunacion_diaria["Vacunados"].mean()
            ) * 100
            if cv > 50:
                insights_temporales.append(
                    f"⚠️ **Alta variabilidad diaria** (CV: {cv:.1f}%) - considerar estandarizar proceso"
                )
            elif cv < 25:
                insights_temporales.append(
                    f"✅ **Ritmo consistente** (CV: {cv:.1f}%) - proceso bien establecido"
                )
        ...
        if len(vacunacion_diaria) >= 14 and "analysis" in locals():
            if analysis["trend_slope"] > 1 and analysis["trend_p_value"] < 0.05:
                insights_temporales.append(
                    "🚀 **Aceleración significativa** en el ritmo de vacunación"
                )
            elif analysis["trend_slope"] < -1 and analysis["trend_p_value"] < 0.05:
                insights_temporales.append(
                    "📉 **Desaceleración significativa** - revisar estrategia"
                )
        ...
        if insights_temporales:
            ...
            st.info(
                "📊 Los patrones temporales muestran un comportamiento estable sin alertas significativas."
            )
...
def mostrar_trends():
    df_combined, _, fecha_corte = load_and_combine_data(
        "data/Resumen.xlsx",
        "data/vacunacion_fa.csv",
        "data/Poblacion_aseguramiento.xlsx",
    )

    st.header("Tendencias de Vacunación")

    # Tendencia diaria
    daily_trend = (
        df_combined.groupby("Fecha_Aplicacion").size().reset_index(name="Vacunados")
    )
    fig_daily = px.line(
        daily_trend, x="Fecha_Aplicacion", y="Vacunados", title="Vacunación Diaria"
    )
    st.plotly_chart(fig_daily)

    # Comparación pre vs post emergencia
    col1, col2 = st.columns(2)
    with col1:
        pre_emergency = df_combined[df_combined["Fecha_Aplicacion"] < fecha_corte]
        st.metric(
            "Promedio Diario Pre-Emergencia",
            f"{pre_emergency.groupby('Fecha_Aplicacion').size().mean():.0f}",
        )

    with col2:
        post_emergency = df_combined[df_combined["Fecha_Aplicacion"] >= fecha_corte]
        st.metric(
            "Promedio Diario Durante Emergencia",
            f"{post_emergency.groupby('Fecha_Aplicacion').size().mean():.0f}",
        )
