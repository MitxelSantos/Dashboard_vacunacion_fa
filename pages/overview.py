...
from src.visualization.charts import create_bar_chart, create_pie_chart, create_scatter_plot
...
def show(data, filters, colors, fuente_poblacion="DANE"):
    ...
    with col1:
        ...
        with col1_1:
            st.metric(
                ...
                value=f"{total_poblacion:,}".replace(",", ".")
            )
    ...
    with col2:
        ...
        comparativa = {
            ...
            "Cobertura (%)": [
                (filtered_data["metricas"]["Vacunados"].sum() / filtered_data["metricas"]["DANE"].sum() * 100) if filtered_data["metricas"]["DANE"].sum() > 0 else 0,
                (filtered_data["metricas"]["Vacunados"].sum() / filtered_data["metricas"]["SISBEN"].sum() * 100) if filtered_data["metricas"]["SISBEN"].sum() > 0 else 0
            ]
        }
    ...
    with col_left:
        ...
        top_municipios = filtered_data["metricas"].sort_values(by=cobertura_col, ascending=False).head(10)
