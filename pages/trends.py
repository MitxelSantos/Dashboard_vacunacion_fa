...
def show(data, filters, colors, fuente_poblacion="DANE"):
    ...
    if "FechaVacunacion" in filtered_data["vacunacion"].columns:
        ...
        if not pd.api.types.is_datetime64_any_dtype(filtered_data["vacunacion"]["FechaVacunacion"]):
            try:
                filtered_data["vacunacion"]["FechaVacunacion"] = pd.to_datetime(filtered_data["vacunacion"]["FechaVacunacion"])
            except:
                st.error("No se pudieron convertir las fechas de vacunación a formato datetime")
                return
        
        # Agrupar por fecha
        vacunacion_diaria = filtered_data["vacunacion"].groupby(filtered_data["vacunacion"]["FechaVacunacion"].dt.date).size().reset_index()
        ...
        if total_poblacion > 0:
            vacunacion_diaria["Cobertura_Acumulada"] = (vacunacion_diaria["Vacunados_Acumulados"] / total_poblacion * 100).round(2)
        ...
        # Extraer mes y año
        filtered_data["vacunacion"]["Mes_Año"] = filtered_data["vacunacion"]["FechaVacunacion"].dt.strftime("%Y-%m")
        
        # Agrupar por mes
        vacunacion_mensual = filtered_data["vacunacion"].groupby("Mes_Año").size().reset_index()
        ...
        st.warning("No se encontraron datos de fecha de vacunación para realizar análisis de tendencias")