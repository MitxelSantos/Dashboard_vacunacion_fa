...
def show(data, filters, colors, fuente_poblacion="DANE"):
    ...
    with col2:
        ...
        filtered_data["metricas"]["Diferencia_Porcentual"] = (
            (filtered_data["metricas"]["SISBEN"] - filtered_data["metricas"]["DANE"])
            / filtered_data["metricas"]["DANE"]
            * 100
        ).round(2)
    ...
    # Preparar tabla
    tabla_datos = filtered_data["metricas"][
        [
            "DPMP",
            "DANE",
            "SISBEN",
    ...
    tabla_datos["Diferencia_Porcentual"] = (
        (tabla_datos["SISBEN"] - tabla_datos["DANE"]) / tabla_datos["DANE"] * 100
    ).round(2)
    ...
    st.dataframe(
        tabla_datos.style.format(
            {
                "DANE": lambda x: format_with_dots(x),
                "SISBEN": lambda x: format_with_dots(x),
                "Vacunados": lambda x: format_with_dots(x),
                "Cobertura_DANE": "{:.2f}%",
                "Cobertura_SISBEN": "{:.2f}%",
                "Pendientes_DANE": lambda x: format_with_dots(x),
                "Pendientes_SISBEN": lambda x: format_with_dots(x),
                "Diferencia_Poblacion": lambda x: f"{x:+,.0f}".replace(",", "."),
