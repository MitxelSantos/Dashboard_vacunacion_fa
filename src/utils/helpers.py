...
class DashboardHelpers:
    ...
    def configure_page_optimized(
        self, page_title: str, page_icon: str = "💉", layout: str = "wide"
    ):
        """
    ...
    def format_number_institutional(
        self,
        number: Union[int, float],
        decimal_places: int = 0,
        show_thousands: bool = True,
    ) -> str:
        """
        ...
        try:
            ...
            if decimal_places == 0:
                ...
                formatted = (
                    ...
                    if show_thousands
                    else f"{number:.{decimal_places}f}"
                )
    ...
    def create_summary_cards(
        self, summary_data: Dict[str, Any], columns: int = 4
    ) -> str:
        """
        ...
        for key, info in summary_data.items():
            ...
            card_html = self.create_metric_card(
                title, value, delta, delta_type, help_text
            )
        ...
        for i in range(0, len(cards_html), cards_per_row):
            ...
            for card in row_cards:
                row_html += (
                    f'<div style="flex: 1; min-width: {row_width-5}%;">{card}</div>'
                )
            row_html += "</div>"
    ...
    def create_data_quality_report(self, data_dict: Dict[str, pd.DataFrame]) -> str:
        ...
        for name, df in data_dict.items():
            if df is not None and len(df) > 0:
                ...
                report_html.append(
                    f"<li><strong>Registros:</strong> {self.format_number_institutional(total_rows)}</li>"
                )
                report_html.append(f"<li><strong>Columnas:</strong> {total_cols}</li>")
                report_html.append(
                    f"<li><strong>Valores nulos:</strong> {null_percentage:.1f}%</li>"
                )
                ...
                report_html.append(f"<h5>{name}</h5>")
                report_html.append(
                    '<p style="color: #dc3545;">❌ Sin datos o DataFrame vacío</p>'
                )
    ...
    def safe_divide(
        self, numerator: float, denominator: float, default_value: float = 0
    ) -> float:
        """
...
def create_metric(title, value, delta=None, delta_type="neutral", help_text=None):
    ...
    return dashboard_helpers.create_metric_card(
        title, value, delta, delta_type, help_text
    )
...
def test_helpers():
    ...
    download_html = create_download(
        test_df, "datos_prueba.csv", "Descargar CSV de Prueba"
    )
