...
def create_metrics_dataframe(df_vacunacion, df_poblacion=None, fuente_poblacion="DANE"):
    ...
    try:
        ...
        metricas = (
            df_vacunacion.groupby(municipio_col)
            .agg({df_vacunacion.columns[0]: "count"})  # Contar registros
            .reset_index()
        )
        ...
        if df_poblacion is not None and not df_poblacion.empty:
            try:
                ...
                if pop_municipio_col and fuente_poblacion in df_poblacion.columns:
                    ...
                    metricas_norm = normalize_municipality_names(
                        metricas.copy(), "DPMP"
                    )
                    poblacion_norm = normalize_municipality_names(
                        df_poblacion.copy(), pop_municipio_col
                    )

                    # Fusionar datos
                    metricas_combined = pd.merge(
                        metricas_norm,
                        poblacion_norm[["DPMP", fuente_poblacion]]
                        .groupby("DPMP")[fuente_poblacion]
                        .sum()
                        .reset_index(),
                        on="DPMP",
                        how="left",
                    ...
                    # Calcular cobertura
                    metricas_combined[fuente_poblacion] = metricas_combined[
                        fuente_poblacion
                    ].fillna(0)
                    metricas_combined[f"Cobertura_{fuente_poblacion}"] = np.where(
                        metricas_combined[fuente_poblacion] > 0,
                        (
                            metricas_combined["Vacunados"]
                            / metricas_combined[fuente_poblacion]
                            * 100
                        ).round(2),
                        0,
                    )
...
def prepare_dashboard_data(df_vacunacion, df_poblacion=None, fuente_poblacion="DANE"):
    ...
    try:
        ...
        dashboard_data = {
            "vacunacion": (
                df_vacunacion if df_vacunacion is not None else pd.DataFrame()
            ),
            "poblacion": df_poblacion if df_poblacion is not None else pd.DataFrame(),
            "metricas": create_metrics_dataframe(
                df_vacunacion, df_poblacion, fuente_poblacion
            ),
        }
