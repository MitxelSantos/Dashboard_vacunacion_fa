...
def apply_filters(data, filters, fuente_poblacion="DANE"):
    ...
    if filters["municipio"] != "Todos":
        vacunacion_df = vacunacion_df[vacunacion_df["NombreMunicipioResidencia"] == filters["municipio"]]
    
    # Aplicar filtro de grupo de edad
    if filters["grupo_edad"] != "Todos":
        vacunacion_df = vacunacion_df[vacunacion_df["Grupo_Edad"] == filters["grupo_edad"]]
    ...
    if filters["grupo_etnico"] != "Todos":
        vacunacion_df = vacunacion_df[vacunacion_df["GrupoEtnico"] == filters["grupo_etnico"]]
    
    # Aplicar filtro de régimen
    if filters["regimen"] != "Todos":
        vacunacion_df = vacunacion_df[vacunacion_df["RegimenAfiliacion"] == filters["regimen"]]
    
    # Aplicar filtro de aseguradora
    if filters["aseguradora"] != "Todos":
        vacunacion_df = vacunacion_df[vacunacion_df["NombreAseguradora"] == filters["aseguradora"]]
