...
class UnifiedDataCleaner:
    ...
    def __init__(self):
        ...
        self.gender_mapping = {
            'masculino': 'Masculino', 'm': 'Masculino', 'masc': 'Masculino', 
            'hombre': 'Masculino', 'h': 'Masculino', 'male': 'Masculino', '1': 'Masculino',
            'femenino': 'Femenino', 'f': 'Femenino', 'fem': 'Femenino', 
            'mujer': 'Femenino', 'female': 'Femenino', '2': 'Femenino'
        }
        ...
        self.null_values = [
            ...
            '#N/A', 'N/A', 'Sin dato', 'sin dato', 'SIN DATO', np.nan, None
        ]
    ...
    def _clean_population_data(self, df):
        ...
        if municipio_cols:
            ...
            if df_clean[municipio_col].astype(str).str.contains(' - ').any():
                df_clean[f'{municipio_col}_codigo'] = df_clean[municipio_col].astype(str).str.split(' - ').str[0]
                df_clean[f'{municipio_col}_nombre'] = df_clean[municipio_col].astype(str).str.split(' - ').str[1]
                
                # Normalizar nombres de municipios
                df_clean[f'{municipio_col}_nombre_norm'] = df_clean[f'{municipio_col}_nombre'].str.lower().str.strip()
                ...
                self.cleaning_stats['municipalities_normalized'] += len(self.municipality_mapping)
        
        # Buscar columna de EAPB
        eapb_cols = [col for col in df_clean.columns if 'eapb' in col.lower() or 'aseguradora' in col.lower()]
        
        if eapb_cols:
            eapb_col = eapb_cols[0]
            # Normalizar nombres de EAPB
            df_clean[f'{eapb_col}_normalized'] = df_clean[eapb_col].astype(str).str.strip().str.title()
            self.cleaning_stats['eapb_normalized'] += len(df_clean)
        
        # Convertir columnas numéricas
        numeric_cols = []
        for col in df_clean.columns:
            if any(word in col.lower() for word in ['total', 'contributivo', 'subsidiado', 'especial']):
                try:
    ...
    def _clean_vaccination_data(self, df):
        ...
        # Normalizar género
        gender_cols = [col for col in df_clean.columns if col.lower() in ['sexo', 'genero', 'género']]
        
        for gender_col in gender_cols:
            if gender_col in df_clean.columns:
                df_clean[f'{gender_col}_normalized'] = df_clean[gender_col].astype(str).str.lower().str.strip()
                ...
                mask_unmapped = ~df_clean[f'{gender_col}_normalized'].isin(mapped_values)
                df_clean.loc[mask_unmapped, f'{gender_col}_normalized'] = 'No Binario'
                
                self.cleaning_stats['gender_normalized'] += len(df_clean)
        
        # Normalizar nombres de municipios
        municipio_cols = [col for col in df_clean.columns if 'municipio' in col.lower() and 'residencia' in col.lower()]
        
        for municipio_col in municipio_cols:
            if municipio_col in df_clean.columns:
                df_clean[f'{municipio_col}_normalized'] = df_clean[municipio_col].astype(str).str.lower().str.strip()
        ...
        # Procesar fechas
        date_cols = [col for col in df_clean.columns if 'fecha' in col.lower() or col in ['FA UNICA']]
        
        for date_col in date_cols:
            if date_col in df_clean.columns:
                df_clean[f'{date_col}_processed'] = pd.to_datetime(df_clean[date_col], errors='coerce')
                self.cleaning_stats['dates_processed'] += df_clean[f'{date_col}_processed'].notna().sum()
        
        # Normalizar columnas categóricas comunes
        categorical_cols = [
            'RegimenAfiliacion', 'GrupoEtnico', 'NombreAseguradora', 
        ...
        for cat_col in categorical_cols:
            if cat_col in df_clean.columns:
                ...
                if cat_col in ['Desplazado', 'Discapacitado']:
                    df_clean[cat_col] = df_clean[cat_col].astype(str).str.lower().str.strip()
                    boolean_mapping = {
                        'true': 'Sí', '1': 'Sí', 'si': 'Sí', 'sí': 'Sí', 'yes': 'Sí', 'y': 'Sí',
    ...
    def _clean_brigades_data(self, df):
            ...
            for col in df_clean.columns:
                ...
                if any(name in col_normalized for name in possible_names):
                    if standard_name not in df_clean.columns:
                        df_clean[standard_name] = pd.to_numeric(df_clean[col], errors='coerce').fillna(0)
                    break
        ...
        for loc_col in location_cols:
            if loc_col in df_clean.columns:
                df_clean[f'{loc_col}_normalized'] = df_clean[loc_col].astype(str).str.strip().str.title()
        
        # Normalizar fechas
        if 'FECHA' in df_clean.columns:
            df_clean['FECHA_processed'] = pd.to_datetime(df_clean['FECHA'], errors='coerce')
            self.cleaning_stats['dates_processed'] += df_clean['FECHA_processed'].notna().sum()
    ...
    def _show_cleaning_summary(self):
        ...
        with col1:
            st.metric("Columnas normalizadas", self.cleaning_stats['columns_normalized'])
        ...
        with col3:
            st.metric("Municipios normalizados", self.cleaning_stats['municipalities_normalized'])
...
def test_data_cleaner():
    ...
    test_data = {
        'test_vaccination': pd.DataFrame({
            ...
            'NombreMunicipioResidencia': ['IBAGUÉ', 'san Sebastian de mariquita', '', 'HONDA', 'Armero Guayabal'],
            ' Edad_Vacunacion ': [25, 30, '', 45, 55],
            'FA UNICA': ['2024-01-15', '2024-02-20', '', '2024-03-10', '2024-04-05']
        }),
